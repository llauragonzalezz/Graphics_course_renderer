//
// ConstructiveSolidIntersection.hpp
//
// Description:
//  Figure intersection wrapper
//
// Authors:
//  Samuel García
//  Laura González
...
#ifndef INFORMATICA_GRAFICA_CONSTRUCTIVESOLIDINTERSECTION_HPP
#define INFORMATICA_GRAFICA_CONSTRUCTIVESOLIDINTERSECTION_HPP

#include <memory>
#include "Figure.hpp"

class ConstructiveSolidIntersection : public Figure {

public:
    std::shared_ptr<Figure> fig1;
    std::shared_ptr<Figure> fig2;

    ConstructiveSolidIntersection() {};

    ConstructiveSolidIntersection(std::shared_ptr<Figure> _fig1, std::shared_ptr<Figure> _fig2){
        fig1 = _fig1;
        fig2 = _fig2;
    }
    ...
    HitRegister collides(const Ray &ray) const override {
        ...
        if (reg1.hits && reg2.hits){
            if (reg1.t < reg2.t && reg1.t_max > reg2.t_max) return reg2;
            else if (reg2.t < reg1.t  && reg2.t_max > reg1.t_max) return reg1;
        }
...
#endif //INFORMATICA_GRAFICA_CONSTRUCTIVESOLIDINTERSECTION_HPP
